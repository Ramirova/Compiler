...
class SemanticAnalyser(HelloVisitor):
    ...
    def visitLang_range(self, ctx):
        start_range = ctx.children[0].getText()
        end_range = ctx.children[1].getText()
        start_range = unicodedata.normalize('NFKD', start_range).encode('ascii', 'ignore')
        end_range = unicodedata.normalize('NFKD', end_range).encode('ascii', 'ignore')
